...
function BrowseCarers() {
  ...
  return (
    <Container>
      ...
      <div
        className="scrollbar-none relative -mx-4 mt-3 flex gap-2 overflow-x-auto px-4 pb-1 sm:-mx-6 sm:px-6 md:mx-0 md:flex-wrap md:overflow-visible md:px-0"
        role="group"
        aria-label="Filters"
      >
      ...
      <div className="mt-5 flex min-h-11 items-center justify-between gap-3">
        <p className="text-sm text-ink-muted" aria-live="polite">
          {query.data
            ? `${count} ${count === 1 ? "carer" : "carers"} available${query.isFetching ? ", updating…" : ""}`
            : query.isError
              ? ""
              : "Looking for carers…"}
        </p>
      ...
      <div className="mt-2">
        <QueryView
          query={query}
          loading={
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3" role="status" aria-label="Loading carers">
        ...
        >
          {(earners) => (
            <ul className="grid gap-4 md:grid-cols-2 xl:grid-cols-3" aria-busy={query.isFetching}>
              {earners.map((earner) => (
                <li key={earner.id} className="flex">
