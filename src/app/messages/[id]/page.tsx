...
function Thread({ thread }: { thread: ChatThread }) {
  ...
  return (
    <div className="flex h-[calc(100dvh-9rem)] min-h-[28rem] flex-col overflow-hidden rounded-card border border-hairline bg-canvas sm:h-[calc(100dvh-12rem)]">
      <div className="flex items-center gap-3 border-b border-hairline bg-surface px-3 py-3 sm:px-5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <Link href={`/bookings/${thread.bookingId}`} className="block truncate text-small text-leash-dark hover:underline">
            {catalogue.label(thread.service)} · {formatDayTime(thread.bookingStart)} · {thread.bookingCode}
          </Link>
        </div>
      </div>
      <p className="flex items-center justify-center gap-1.5 border-b border-hairline bg-sky/60 px-3 py-2 text-center text-small text-leash-dark">
...
export default function ChatThreadPage() {
  ...
  return (
    <div className="flex h-[28rem] flex-col gap-4 rounded-card border border-hairline bg-surface p-5" role="status" aria-label="Loading conversation">
